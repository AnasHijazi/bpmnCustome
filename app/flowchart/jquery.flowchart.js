    ...
    // "colorize" the widget name
        $.widget("flowchart.flowchart", {
            ...
            _create: function () {
                ...
                this.objs.layers.msg = $('<span class="flowchart-tooltip"></span>');
                this.objs.layers.msg.appendTo(this.element);
            ...
            _initEvents: function () {
                ...
                //JMH - 02-10-2018 - Added events for deleting/unselecting stuff
                $(".flowchart-example").keydown(function (e) {
                    var evt = e || window.event;
                    var key = evt.keyCode || evt.charCode;

                    //Delete/backspace button pressed, delete selected element
                    if (key == 8 || key == 46) {
                        self.deleteSelected();
                    }
                    //Escape key pressed, clear selections
                    if(key === 27) {
                        self.clearSelection();
                    }
                });
                ...
                $(document).mousemove(function (e) {
                    self.objs.layers.msg[0].style.left = e.pageX + 'px';
                    self.objs.layers.msg[0].style.top = e.pageY + 'px';
                });
                ...
                this.objs.layers.operators.on('click', '.flowchart-operator-connector', function () {
                    ...
                    if (self.options.canUserEditLinks) {
                        //JMH - 30-09-2018 - Added value parameter to the function
                        ...
                        self._connectorClicked($this.closest('.flowchart-operator').data('operator_id'), $this.data('connector'), $this.data('sub_connector'), $this.closest('.flowchart-operator-connector-set').data('connector_type'), $this.data('value'));
                    }
                });
            ...
            createLink: function (linkId, linkDataOriginal) {
                //JMH - 30-09-2018 - Added console statement
                // console.log(`Link data: `, linkDataOriginal);
            ...
            _autoCreateSubConnector: function (operator, connector, connectorType, subConnector) {
                //JMH - 26-09-2018
                let tempConnector = connector.split('->');
                // function findConnectorDetails(connectorProperties, callback) {
                //     let details = null;
                //     for(let key in connectorProperties) {
                //         const parentConnector = connectorProperties[key];
                //         if(parentConnector.hasOwnProperty('children')) {
                //             let temp = parentConnector['children'][tempConnector];
                //             if(!temp || temp === undefined) {
                //                 details = findConnectorDetails(parentConnector.children, callback);
                //             } else {
                ...
                if(!connectorInfos || connectorInfos === undefined) {
                    //Search through the children of each connector to find the child
                    // findConnectorDetails(this.data.operators[operator].properties[connectorType], function(result) {
                    //     connectorInfos = result;
                    // });
                    let temp = this.data.operators[operator].properties[connectorType];
                    while(tempConnector.length > 1) {
                        temp = temp[tempConnector[0]].children;
                        tempConnector.shift();
                    }
                    connectorInfos = temp[tempConnector[0]];
                }
                if (connectorInfos.hasOwnProperty( 'multiple') && connectorInfos.multiple) {
            ...
            _getOperatorFullElement: function (operatorData) {
                ...
                //JMH - 26-09-2018 - Commented out the append line and moved it to the bottom before adding inputs/outputs
                ...
                function addConnector(connectorKey, connectorInfos, $operator_container, connectorType) {
                    var $operator_connector_set = $('<div class="flowchart-operator-connector-set" id="'+ connectorKey + '"></div>');
                    $operator_connector_set.data('connector_type', connectorType);
                    $operator_connector_set.appendTo($operator_container);
    
                    //JMH - 26-09-2018 - commented out
                    // connectorArrows[connectorKey] = [];
                    // connectorSmallArrows[connectorKey] = [];
                    // connectors[connectorKey] = [];
                    // connectorSets[connectorKey] = $operator_connector_set;
    
                    //JMH - 26-09-2018 - commented out the first line and added the rest
                    // self._createSubConnector(connectorKey, connectorInfos, fullElement);
    
                    function checkChildren(connectorDetails, conKey, parentConnectorKey) {
                        let tempKey = conKey;
                        if(parentConnectorKey) {
                            tempKey = parentConnectorKey + '->' + conKey;
                        }
                        if(connectorDetails.hasOwnProperty('children')) {
                            connectorArrows[tempKey] = [];
                            connectorSmallArrows[tempKey] = [];
                            connectors[tempKey] = [];
                            connectorSets[tempKey] = $operator_connector_set;
                            self._createSubConnector(tempKey, connectorDetails, fullElement, parentConnectorKey);
                            for(let key in connectorDetails.children) {
                                checkChildren(connectorDetails.children[key], key, tempKey);
                            }
                        } else {
                            connectorArrows[tempKey] = [];
                            connectorSmallArrows[tempKey] = [];
                    ...
                    checkChildren(connectorInfos, connectorKey);
                    // If the connector has children, create a label element and then add the child connectors
                    // if(connectorInfos.hasOwnProperty('children')) {
                    //     connectorSets[connectorKey] = $operator_connector_set;
                    //     self._createParentLabel(connectorKey, connectorInfos, fullElement);
                    //     for(let key in connectorInfos.children) {
                    //         connectorArrows[key] = [];
                    //         connectorSmallArrows[key] = [];
                    //         connectors[key] = [];
                    //         self._createSubConnector(key, connectorInfos.children[key], fullElement, connectorKey);
                    //     }
                    // } else {
                    //     connectorArrows[connectorKey] = [];
                    //         connectorSmallArrows[connectorKey] = [];
                ...
                if (Object.keys(infos.inputs).length > 0) {
                    $operator_inputs.appendTo($operator_inputs_outputs);
                }
                ...
                if (Object.keys(infos.outputs).length > 0) {
                    $operator_outputs.appendTo($operator_inputs_outputs);
                }
            ...
            _createParentLabel: function (connectorKey, connectorInfos, fullElement) {
                var $operator_connector_set = fullElement.connectorSets[connectorKey];
                // console.log(`connectorKey`, connectorKey);
                // console.log(`connectorInfos`, connectorInfos);
                // console.log(`fullElement`, fullElement);
    
                var $operator_parent = $('<div class="flowchart-operator-parent"></div>');
                $operator_parent.appendTo($operator_connector_set);
    
                var $operator_parent_label = $('<div class="flowchart-operator-parent-label"></div>');
                $operator_parent_label.text(connectorInfos.label);
                $operator_parent_label.appendTo($operator_parent);
            },
    
            //JMH - 26-09-2018 - Added a new parameter: parentKey to be used when adding a child connector
            _createSubConnector: function (connectorKey, connectorInfos, fullElement, parentKey) {
                //JMH - 26-09-2018 - Commented out first line and added the if statement
                //var $operator_connector_set = fullElement.connectorSets[connectorKey];
                if(parentKey) {
                    var $operator_connector_set = fullElement.connectorSets[parentKey];
                } else {
                    var $operator_connector_set = fullElement.connectorSets[connectorKey];
                }
                ...
                //JMH - 30-09-2018
                $operator_connector.data('value', connectorInfos.value);
    
                if(!parentKey) {
                    var $operator_connector_label = $('<div class="flowchart-operator-parent-label"></div>');
                    $operator_connector_label.text(' - ' + connectorInfos.label.replace('(:i)', subConnector + 1));
                } else {
                    let childLevel = (connectorKey.match(/->/g) || []).length;
                    var $operator_connector_label = $('<div class="flowchart-operator-connector-label-' + childLevel + '"></div>');
                    $operator_connector_label.text(connectorInfos.label.replace('(:i)', subConnector + 1));
                }
            ...
            //JMH - 02-10-2018 - Added clearMsg function
            clearMsg: function () {
                this.objs.layers.msg.text('');
                this.objs.layers.msg[0].style.display = 'none';
            },
    
            _connectorClicked: function (operator, connector, subConnector, connectorCategory, value) {
                if (connectorCategory == 'outputs') {
                    ...
                    if(this.lastOutputConnectorClicked.type === 'Array') {
                        let subType = Object.prototype.toString.call(value[0]).split(' ')[1];
                        subType = subType.substring(0, subType.length - 1);
                        this.lastOutputConnectorClicked.contents = subType;
                    }
                ...
                if (connectorCategory == 'inputs' && this.lastOutputConnectorClicked != null) {
                    let subType, type = Object.prototype.toString.call(value).split(' ')[1];
                    type = type.substring(0, type.length - 1);
                    ...
                    var linkData = {
                        ...
                        //fromValue: this.lastOutputConnectorClicked.value,
                        fromType: this.lastOutputConnectorClicked.type,
                        toType: type
                    };

                    if(linkData.fromType === 'Array') {
                        linkData.fromContents = this.lastOutputConnectorClicked.contents;
                    }

                    if(linkData.fromType !== linkData.toType && linkData.toType !== 'String') {
                        this.objs.layers.msg.html(`<b>From</b> (<span style="color:red">` + linkData.fromType + `</span>) and <b>To</b> (<span style="color:red">` + linkData.toType + `</span>) types do not match... Cannot make connection`);
                        this.objs.layers.msg[0].style.display = 'block';
                    } else {
                        this.clearMsg();
            ...
            _cleanMultipleConnectors: function (operator, connector, linkFromTo) {
                //JMH - 27-09-2018 - Commented out first if statement and added the function to search for the connector component
                //if (!this.data.operators[operator].properties[linkFromTo == 'from' ? 'outputs' : 'inputs'][connector].multiple) {
                if (this.data.operators[operator].properties[linkFromTo == 'from' ? 'outputs' : 'inputs'][connector] &&
                    this.data.operators[operator].properties[linkFromTo == 'from' ? 'outputs' : 'inputs'][connector].hasOwnProperty('multiple') &&
                    !this.data.operators[operator].properties[linkFromTo == 'from' ? 'outputs' : 'inputs'][connector].multiple) {
                    return;
                }
    
                function findConnectorDetails(connectorProperties) {
                    let details = null;
                    for(let key in connectorProperties) {
                        const parentConnector = connectorProperties[key];
                        if(parentConnector.hasOwnProperty('children')) {
                            let temp = parentConnector['children'][connector];
                            if(!temp || temp === undefined) {
                                return findConnectorDetails(parentConnector.children);
                            } else {
                                details = temp;
                                break;
                            }
