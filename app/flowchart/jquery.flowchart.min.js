$(function(){$.widget("flowchart.flowchart",{options:{canUserEditLinks:!0,canUserMoveOperators:!0,data:{},distanceFromArrow:3,defaultOperatorClass:'flowchart-default-operator',defaultLinkColor:'#3366ff',defaultSelectedLinkColor:'black',linkWidth:5,grid:20,multipleLinksOnOutput:!1,multipleLinksOnInput:!1,linkVerticalDecal:0,onOperatorSelect:function(operatorId){return!0},onOperatorUnselect:function(){return!0},onOperatorMouseOver:function(operatorId){return!0},onOperatorMouseOut:function(operatorId){return!0},onLinkSelect:function(linkId){return!0},onLinkUnselect:function(){return!0},onOperatorCreate:function(operatorId,operatorData,fullElement){return!0},onLinkCreate:function(linkId,linkData){return!0},onOperatorDelete:function(operatorId){return!0},onLinkDelete:function(linkId,forced){return!0},onOperatorMoved:function(operatorId,position){},onAfterChange:function(changeType){}},data:null,objs:null,maskNum:0,linkNum:0,operatorNum:0,lastOutputConnectorClicked:null,selectedOperatorId:null,selectedLinkId:null,positionRatio:1,globalId:null,_create:function(){if(typeof document.__flowchartNumber=='undefined'){document.__flowchartNumber=0}else{document.__flowchartNumber++}
this.globalId=document.__flowchartNumber;this._unitVariables();this.element.addClass('flowchart-container');this.objs.layers.msg=$('<span class="flowchart-tooltip"></span>');this.objs.layers.msg.appendTo(this.element);this.objs.layers.links=$('<svg class="flowchart-links-layer"></svg>');this.objs.layers.links.appendTo(this.element);this.objs.layers.operators=$('<div class="flowchart-operators-layer unselectable"></div>');this.objs.layers.operators.appendTo(this.element);this.objs.layers.temporaryLink=$('<svg class="flowchart-temporary-link-layer"></svg>');this.objs.layers.temporaryLink.appendTo(this.element);var shape=document.createElementNS("http://www.w3.org/2000/svg","line");shape.setAttribute("x1","0");shape.setAttribute("y1","0");shape.setAttribute("x2","0");shape.setAttribute("y2","0");shape.setAttribute("stroke-dasharray","6,6");shape.setAttribute("stroke-width","4");shape.setAttribute("stroke","black");shape.setAttribute("fill","none");this.objs.layers.temporaryLink[0].appendChild(shape);this.objs.temporaryLink=shape;this._initEvents();if(typeof this.options.data!='undefined'){this.setData(this.options.data)}},_unitVariables:function(){this.data={operators:{},links:{}};this.objs={layers:{operators:null,temporaryLink:null,links:null},linksContext:null,temporaryLink:null}},_initEvents:function(){var self=this;$(".flowchart-example").keydown(function(e){var evt=e||window.event;var key=evt.keyCode||evt.charCode;if(key==8||key==46){self.deleteSelected()}
if(key===27){self.clearSelection()}});$(document).mousemove(function(e){self.objs.layers.msg[0].style.left=e.pageX+'px';self.objs.layers.msg[0].style.top=e.pageY+'px'});this.element.mousemove(function(e){var $this=$(this);var offset=$this.offset();self._mousemove((e.pageX-offset.left)/self.positionRatio,(e.pageY-offset.top)/self.positionRatio,e)});this.element.click(function(e){var $this=$(this);var offset=$this.offset();self._click((e.pageX-offset.left)/self.positionRatio,(e.pageY-offset.top)/self.positionRatio,e)});this.objs.layers.operators.on('pointerdown mousedown touchstart','.flowchart-operator',function(e){e.stopImmediatePropagation()});this.objs.layers.operators.on('click','.flowchart-operator',function(e){if($(e.target).closest('.flowchart-operator-connector').length==0){self.selectOperator($(this).data('operator_id'))}});this.objs.layers.operators.on('click','.flowchart-operator-connector',function(){var $this=$(this);if(self.options.canUserEditLinks){self._connectorClicked($this.closest('.flowchart-operator').data('operator_id'),$this.data('connector'),$this.data('sub_connector'),$this.closest('.flowchart-operator-connector-set').data('connector_type'),$this.data('value'))}});this.objs.layers.links.on('mousedown touchstart','.flowchart-link',function(e){e.stopImmediatePropagation()});this.objs.layers.links.on('mouseover','.flowchart-link',function(){self._connecterMouseOver($(this).data('link_id'))});this.objs.layers.links.on('mouseout','.flowchart-link',function(){self._connecterMouseOut($(this).data('link_id'))});this.objs.layers.links.on('click','.flowchart-link',function(){self.selectLink($(this).data('link_id'))});this.objs.layers.operators.on('mouseover','.flowchart-operator',function(e){self._operatorMouseOver($(this).data('operator_id'))});this.objs.layers.operators.on('mouseout','.flowchart-operator',function(e){self._operatorMouseOut($(this).data('operator_id'))})},setData:function(data){this._clearOperatorsLayer();this.data.operatorTypes={};if(typeof data.operatorTypes!='undefined'){this.data.operatorTypes=data.operatorTypes}
...
this.createLink(this.linkNum,linkData);return this.linkNum},createLink:function(linkId,linkDataOriginal){var linkData=$.extend(!0,{},linkDataOriginal);if(!this.callbackEvent('linkCreate',[linkId,linkData])){return}
...
this._refreshInternalProperties(operatorData);var infos=$.extend(!0,{},operatorData.internal.properties);for(var connectorId_i in infos.inputs){if(infos.inputs.hasOwnProperty(connectorId_i)){if(infos.inputs[connectorId_i]==null){delete infos.inputs[connectorId_i]}}}
...
return infos},_getOperatorFullElement:function(operatorData){var infos=this.getOperatorCompleteData(operatorData);var $operator=$('<div class="flowchart-operator"></div>');$operator.addClass(infos.class);var $operator_title=$('<div class="flowchart-operator-title"></div>');$operator_title.html(infos.title);$operator_title.appendTo($operator);var $operator_inputs_outputs=$('<div class="flowchart-operator-inputs-outputs"></div>');$operator_inputs_outputs.appendTo($operator);var $operator_inputs=$('<div class="flowchart-operator-inputs"></div>');var $operator_outputs=$('<div class="flowchart-operator-outputs"></div>');var self=this;var connectorArrows={};var connectorSmallArrows={};var connectorSets={};var connectors={};var fullElement={operator:$operator,title:$operator_title,connectorSets:connectorSets,connectors:connectors,connectorArrows:connectorArrows,connectorSmallArrows:connectorSmallArrows};function addConnector(connectorKey,connectorInfos,$operator_container,connectorType){var $operator_connector_set=$('<div class="flowchart-operator-connector-set" id="'+connectorKey+'"></div>');$operator_connector_set.data('connector_type',connectorType);$operator_connector_set.appendTo($operator_container);function checkChildren(connectorDetails,conKey,parentConnectorKey){let tempKey=conKey;if(parentConnectorKey){tempKey=parentConnectorKey+'->'+conKey}
if(connectorDetails.hasOwnProperty('children')){connectorArrows[tempKey]=[];connectorSmallArrows[tempKey]=[];connectors[tempKey]=[];connectorSets[tempKey]=$operator_connector_set;self._createSubConnector(tempKey,connectorDetails,fullElement,parentConnectorKey);for(let key in connectorDetails.children){checkChildren(connectorDetails.children[key],key,tempKey)}}else{connectorArrows[tempKey]=[];connectorSmallArrows[tempKey]=[];connectors[tempKey]=[];connectorSets[tempKey]=$operator_connector_set;self._createSubConnector(tempKey,connectorDetails,fullElement,parentConnectorKey)}}
checkChildren(connectorInfos,connectorKey)}
if(Object.keys(infos.inputs).length>0){$operator_inputs.appendTo($operator_inputs_outputs)}
...
if(Object.keys(infos.outputs).length>0){$operator_outputs.appendTo($operator_inputs_outputs)}
...
return fullElement},_createParentLabel:function(connectorKey,connectorInfos,fullElement){var $operator_connector_set=fullElement.connectorSets[connectorKey];var $operator_parent=$('<div class="flowchart-operator-parent"></div>');$operator_parent.appendTo($operator_connector_set);var $operator_parent_label=$('<div class="flowchart-operator-parent-label"></div>');$operator_parent_label.text(connectorInfos.label);$operator_parent_label.appendTo($operator_parent)},_createSubConnector:function(connectorKey,connectorInfos,fullElement,parentKey){if(parentKey){var $operator_connector_set=fullElement.connectorSets[parentKey]}else{var $operator_connector_set=fullElement.connectorSets[connectorKey]}
var subConnector=fullElement.connectors[connectorKey].length;var $operator_connector=$('<div class="flowchart-operator-connector"></div>');$operator_connector.appendTo($operator_connector_set);$operator_connector.data('connector',connectorKey);$operator_connector.data('sub_connector',subConnector);$operator_connector.data('value',connectorInfos.value);if(!parentKey){var $operator_connector_label=$('<div class="flowchart-operator-parent-label"></div>');$operator_connector_label.text(' - '+connectorInfos.label.replace('(:i)',subConnector+1))}else{let childLevel=(connectorKey.match(/->/g)||[]).length;var $operator_connector_label=$('<div class="flowchart-operator-connector-label-'+childLevel+'"></div>');$operator_connector_label.text(connectorInfos.label.replace('(:i)',subConnector+1))}
$operator_connector_label.appendTo($operator_connector);var $operator_connector_arrow=$('<div class="flowchart-operator-connector-arrow"></div>');$operator_connector_arrow.appendTo($operator_connector);var $operator_connector_small_arrow=$('<div class="flowchart-operator-connector-small-arrow"></div>');$operator_connector_small_arrow.appendTo($operator_connector);fullElement.connectors[connectorKey].push($operator_connector);fullElement.connectorArrows[connectorKey].push($operator_connector_arrow);fullElement.connectorSmallArrows[connectorKey].push($operator_connector_small_arrow)},getOperatorElement:function(operatorData){var fullElement=this._getOperatorFullElement(operatorData);return fullElement.operator},addOperator:function(operatorData){while(typeof this.data.operators[this.operatorNum]!='undefined'){this.operatorNum++}
this.createOperator(this.operatorNum,operatorData);return this.operatorNum},createOperator:function(operatorId,operatorData){operatorData.internal={};this._refreshInternalProperties(operatorData);var fullElement=this._getOperatorFullElement(operatorData);if(!this.callbackEvent('operatorCreate',[operatorId,operatorData,fullElement])){return!1}
...
if(this.options.canUserMoveOperators){var pointerX;var pointerY;fullElement.operator.draggable({containment:operatorData.internal.properties.uncontained?!1:this.element,handle:'.flowchart-operator-title',start:function(e,ui){if(self.lastOutputConnectorClicked!=null){e.preventDefault();return}
...
this.callbackEvent('afterChange',['operator_create'])},clearMsg:function(){this.objs.layers.msg.text('');this.objs.layers.msg[0].style.display='none'},_connectorClicked:function(operator,connector,subConnector,connectorCategory,value){if(connectorCategory=='outputs'){var d=new Date();let type=Object.prototype.toString.call(value).split(' ')[1];type=type.substring(0,type.length-1);this.lastOutputConnectorClicked={operator:operator,connector:connector,subConnector:subConnector,type:type};if(this.lastOutputConnectorClicked.type==='Array'){let subType=Object.prototype.toString.call(value[0]).split(' ')[1];subType=subType.substring(0,subType.length-1);this.lastOutputConnectorClicked.contents=subType}
this.objs.layers.temporaryLink.show();var position=this.getConnectorPosition(operator,connector,subConnector);var x=position.x+position.width;var y=position.y;this.objs.temporaryLink.setAttribute('x1',x.toString());this.objs.temporaryLink.setAttribute('y1',y.toString());this._mousemove(x,y)}
if(connectorCategory=='inputs'&&this.lastOutputConnectorClicked!=null){let subType,type=Object.prototype.toString.call(value).split(' ')[1];type=type.substring(0,type.length-1);var linkData={fromOperator:this.lastOutputConnectorClicked.operator,fromConnector:this.lastOutputConnectorClicked.connector,fromSubConnector:this.lastOutputConnectorClicked.subConnector,toOperator:operator,toConnector:connector,toSubConnector:subConnector,fromType:this.lastOutputConnectorClicked.type,toType:type};if(linkData.fromType==='Array'){linkData.fromContents=this.lastOutputConnectorClicked.contents}
if(linkData.fromType!==linkData.toType&&linkData.toType!=='String'){this.objs.layers.msg.html(`<b>From</b> (<span style="color:red">`+linkData.fromType+`</span>) and <b>To</b> (<span style="color:red">`+linkData.toType+`</span>) types do not match... Cannot make connection`);this.objs.layers.msg[0].style.display='block'}else{this.clearMsg();this.addLink(linkData);this._unsetTemporaryLink()}}},clearSelection:function(){this.clearMsg();this._unsetTemporaryLink();this.unselectOperator();this.unselectLink()},_unsetTemporaryLink:function(){this.clearMsg();this.lastOutputConnectorClicked=null;this.objs.layers.temporaryLink.hide()},_mousemove:function(x,y,e){if(this.lastOutputConnectorClicked!=null){this.objs.temporaryLink.setAttribute('x2',x);this.objs.temporaryLink.setAttribute('y2',y)}},_click:function(x,y,e){var $target=$(e.target);if($target.closest('.flowchart-operator-connector').length==0){this._unsetTemporaryLink()}
...
this._removeSelectedClassOperators();this.selectedOperatorId=null}},_addSelectedClass:function(operatorId){this.data.operators[operatorId].internal.els.operator.addClass('selected')},callbackEvent:function(name,params){var cbName='on'+name.charAt(0).toUpperCase()+name.slice(1);var ret=this.options[cbName].apply(this,params);if(ret!==!1){var returnHash={'result':ret}
this.element.trigger(name,params.concat([returnHash]));ret=returnHash.result}
...
this.unselectOperator();this.selectedLinkId=linkId;this.colorizeLink(linkId,this.options.defaultSelectedLinkColor)},deleteOperator:function(operatorId){this._deleteOperator(operatorId,!1)},_deleteOperator:function(operatorId,replace){if(!this.callbackEvent('operatorDelete',[operatorId,replace])){return!1}
if(!replace){for(var linkId in this.data.links){if(this.data.links.hasOwnProperty(linkId)){var currentLink=this.data.links[linkId];if(currentLink.fromOperator==operatorId||currentLink.toOperator==operatorId){this._deleteLink(linkId,!0)}}}}
if(!replace&&operatorId==this.selectedOperatorId){this.unselectOperator()}
this.data.operators[operatorId].internal.els.operator.remove();delete this.data.operators[operatorId];this.callbackEvent('afterChange',['operator_delete'])},deleteLink:function(linkId){this._deleteLink(linkId,!1)},_deleteLink:function(linkId,forced){if(this.selectedLinkId==linkId){this.unselectLink()}
if(!this.callbackEvent('linkDelete',[linkId,forced])){if(!forced){return}}
this.colorizeLink(linkId,'transparent');var linkData=this.data.links[linkId];var fromOperator=linkData.fromOperator;var fromConnector=linkData.fromConnector;var toOperator=linkData.toOperator;var toConnector=linkData.toConnector;linkData.internal.els.overallGroup.remove();delete this.data.links[linkId];this._cleanMultipleConnectors(fromOperator,fromConnector,'from');this._cleanMultipleConnectors(toOperator,toConnector,'to');this.callbackEvent('afterChange',['link_delete'])},_cleanMultipleConnectors:function(operator,connector,linkFromTo){if(this.data.operators[operator].properties[linkFromTo=='from'?'outputs':'inputs'][connector]&&this.data.operators[operator].properties[linkFromTo=='from'?'outputs':'inputs'][connector].hasOwnProperty('multiple')&&!this.data.operators[operator].properties[linkFromTo=='from'?'outputs':'inputs'][connector].multiple){return}
function findConnectorDetails(connectorProperties){let details=null;for(let key in connectorProperties){const parentConnector=connectorProperties[key];if(parentConnector.hasOwnProperty('children')){let temp=parentConnector.children[connector];if(!temp||temp===undefined){return findConnectorDetails(parentConnector.children)}else{details=temp;break}}}
return details}
...
if(this.selectedOperatorId!=null){this.deleteOperator(this.selectedOperatorId)}},setPositionRatio:function(positionRatio){this.positionRatio=positionRatio},getPositionRatio:function(){return this.positionRatio},getData:function(){var keys=['operators','links'];var data={};data.operators=$.extend(!0,{},this.data.operators);data.links=$.extend(!0,{},this.data.links);for(var keyI in keys){if(keys.hasOwnProperty(keyI)){var key=keys[keyI];for(var objId in data[key]){if(data[key].hasOwnProperty(objId)){delete data[key][objId].internal}}}}
